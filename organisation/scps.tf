...
module "scp_deny_root_user" {
  ...
  policy_json = file("${path.module}/scp-policies/deny-root-user.json")
...
module "scp_deny_leave_org" {
  ...
  policy_json = file("${path.module}/scp-policies/deny-leave-org.json")
  target_ids  = [local.root_id]
}
...
module "scp_deny_disable_cloudtrail" {
  ...
  policy_json = file("${path.module}/scp-policies/deny-disable-cloudtrail.json")
  target_ids  = [local.root_id]
}
...
module "scp_deny_disable_config" {
  ...
  policy_json = file("${path.module}/scp-policies/deny-disable-config.json")
  target_ids  = [local.root_id]
}
...
module "scp_deny_disable_guardduty" {
  ...
  policy_json = file("${path.module}/scp-policies/deny-disable-guardduty.json")
  target_ids  = [local.root_id]
}
...
module "scp_restrict_regions" {
  ...
  policy_json = templatefile("${path.module}/scp-policies/restrict-regions.json.tpl", {
...
module "scp_protect_log_archive" {
  ...
  policy_json = file("${path.module}/scp-policies/protect-log-archive.json")
...
module "scp_deny_expensive_services" {
  ...
  policy_json = file("${path.module}/scp-policies/deny-expensive-services.json")
...
module "scp_deny_network_bridging" {
  ...
  policy_json = file("${path.module}/scp-policies/deny-network-bridging.json")
  target_ids  = [module.org_units["Sandbox"].ou_id]
}
